...
from .utils import find_path, read_single_pdf
...
SAVE_JOB_DESCRIPTION_TO = os.path.join(
...
class Processor:
    def __init__(self, input_file, file_type):
        ...
        if file_type == "resume":
            self.input_file_name = os.path.join(
                READ_RESUME_FROM + self.input_file)
    ...
    def _write_json_file(self, data_dict: dict):
        file_name = str(
            f"{self.file_type}_" + self.input_file +
            data_dict["unique_id"] + ".json"
        )
        ...
        if self.file_type == "resume":
            ...
            save_directory_name = pathlib.Path(
                SAVE_JOB_DESCRIPTION_TO) / file_name
